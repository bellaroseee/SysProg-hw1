...
// Static function to help find, remove and insert into hashtable.
// iterate through the HashTable and return true when a matching key is found.
// return false otherwise.
static bool HashTable_FindKey(LLIterator* iter,
                              HTKey_t key,
                              LinkedList* chain,
...
bool HashTable_Insert(HashTable *table,
  ...
  HTKeyValue_t *newKVPair = (HTKeyValue_t*) malloc(sizeof(HTKeyValue_t));
  Verify333(newKVPair != NULL);
  newKVPair->key = newkeyvalue.key;
  newKVPair->value = newkeyvalue.value;

  // chain is empty
  if (LinkedList_NumElements(chain) == 0) {
    LinkedList_Append(chain, (LLPayload_t*) newKVPair);
    table->num_elements += 1;
    return false;
  }

  // chain is not empty
  LLIterator *chainIter = LLIterator_Allocate(chain);
  HTKeyValue_t *oldKVPair;

  if (HashTable_FindKey(chainIter, newkeyvalue.key, chain, &oldKVPair)) {
    LinkedList_Append(chain, (LLPayload_t *) newKVPair);
    oldkeyvalue->key = oldKVPair->key;
    oldkeyvalue->value = oldKVPair->value;
    free(oldKVPair);
...
bool HashTable_Find(HashTable *table,
  ...
  // find the bucket to search from
  ...
  if (HashTable_FindKey(chainIter, key, chain, &oldKVPair)) {
      keyvalue->key = oldKVPair->key;
      keyvalue->value = oldKVPair->value;
      LLIterator_Free(chainIter);
      return true;
  }
...
bool HashTable_Remove(HashTable *table,
  ...
  LLIterator *chainIter = LLIterator_Allocate(chain);
  HTKeyValue_t *returnKVPair;

  if (HashTable_FindKey(chainIter, key, chain, &returnKVPair)) {
      keyvalue->key = returnKVPair->key;
      keyvalue->value = returnKVPair->value;
      free(returnKVPair);
      LLIterator_Remove(chainIter, LLNoOpFree);
      LLIterator_Free(chainIter);
      table->num_elements -= 1;
...
static bool HashTable_FindKey(LLIterator* iter,
  ...
  // chain is not empty
  for (i = 0; i < LinkedList_NumElements(chain); i++) {
    LLIterator_Get(iter, (LLPayload_t*) keyvalue);

    // there is an existing KV pair
    if ((*keyvalue)->key == key) {
      return true;
    }
...
bool HTIterator_Next(HTIterator *iter) {
  ...
  // if it's not the last element in linkedlist
  // iterate to next element in list
  if (LLIterator_Next(iter->bucket_it)) {
    return true;
  }

  // if it's the last element in linkedlist
  // go to the next bucket that is not empty
...
bool HTIterator_Get(HTIterator *iter, HTKeyValue_t *keyvalue) {
  ...
  // trying to get from empty table
  if (!HTIterator_IsValid(iter)) {
    return false;
  }

  HTKeyValue_t *temp;
  LLIterator_Get(iter->bucket_it, (LLPayload_t *) &temp);
